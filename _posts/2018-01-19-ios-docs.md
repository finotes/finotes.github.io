...
@implementation LoginViewController
	...
	-(void)initiateFacebookLogin {
	    ...
	    [loginManager logInWithReadPermissions:@[@"email",@"public_profile",@"publish_actions"] fromViewController:self 
		    ...
		    [self makeLoginApiCall:fbToken withUserId:userId];
	    }];
	}
	...
	- (Boolean) makeLoginApiCall:(NSString *)fbToken withUserId:(NSString *) userId {
	    NSArray *approvedPermissions = [[[FBSDKAccessToken currentAccessToken]permissions] allObjects];
	    if([approvedPermissions containsObject:@"publish_actions"]){
		[[[LoginAPI alloc] init] call];
		return YES;
	    }else{
		[SSQNotify showError:[WCts loginFailed] forFrame:self.view];
		return NO;
	    }
